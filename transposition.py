...
def brute_force(text, likely_words=LIKELY_WORDS, kwlens=None, ask=True):
...
def get_rows(ct, kwlen):
    return [ct[i:i+kwlen] for i in range(0, len(ct), kwlen)]

def print_columns(ct, kwlen):
    rows = get_rows(ct, kwlen)
    print('\n'.join(rows))

def decipher_ez(ct, order, human=False):
    rows = get_rows(ct, len(order))
    columns = [""]*len(order)
    for row in rows:
    ...
    full = ""
    for new_row in range(len(rows)):
        nr = ""
        for order_thing in order:
            nr += columns[order_thing][new_row]
        if human:
            print(nr)
